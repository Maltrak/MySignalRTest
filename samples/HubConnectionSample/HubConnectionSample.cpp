...
void send_message(signalr::hub_proxy proxy, const utility::string_t& name, const utility::string_t& message)
{
	...
	proxy.invoke<void>(U("invoke"), args/*, [](const web::json::value&){}*/)
...
{
	signalr::hub_connection connection{ U("http://localhost:34282") };
	auto proxy = connection.create_hub_proxy(U("messageReceivingHub"));
	proxy.on(U("addInvoke"), [](const web::json::value& m)
	{
